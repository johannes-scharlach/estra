...
import { clearPlannedMeal, movePlannedMeal, setPlannedMeal } from "@/db/planned-meals";
...
export default function Meals() {
  ...
  const { data: planned } = useQuery<PlannedMeal>(
    list ? "SELECT * FROM planned_meals WHERE list_id = ?" : "SELECT * FROM planned_meals WHERE 0",
    list ? [list.id] : [],
  );
  const { data: variants } = useQuery<DbVariant>("SELECT * FROM variants ORDER BY created_at DESC LIMIT 20");
  ...
  const hasPlanOn = (key: string) => {
    if (Object.entries(pending).some(([k, v]) => k.startsWith(`${key}:`) && v)) return true;
    return planned.some((p) => p.slot_date === key);
  };
  ...
  useEffect(() => {
    for (const [k, p] of Object.entries(pending)) {
      ...
      const dbVariant: DisplayRecipe | null = !row || !row.variant_id
        ? null
        : (() => {
            const v = variantById.get(row.variant_id);
            return v && v.name ? { id: v.id, name: v.name } : { id: row.variant_id, name: "…" };
          })();
      if ((p === null && dbVariant === null) || (p?.id && dbVariant?.id === p.id)) {
  ...
  async function doImport() {
    ...
    try {
      ...
      const json = (await res.json()) as { id?: string; variantId?: string; recipe?: unknown; error?: string };
    ...
    <>
      ...
      >
        ...
        <View className="mt-6 flex-row items-baseline gap-2 px-6">
          <Text variant="large">{isToday ? "Today" : WEEKDAY_LONG[selDate.getDay()]}</Text>
          <Text variant="muted">
        ...
        <View className="mt-6 gap-8">
          {SLOT_ORDER.map((slot) => {
            ...
            if (!recipe && !isOpen) {
              return (
                <Pressable key={slot} onPress={() => setOpen(slot, true)} className="flex-row items-center gap-2 px-6">
      ...
      <Modal visible={!!menu} transparent animationType="slide" onRequestClose={closeMenu}>
        <Pressable className="flex-1 justify-end bg-black/40" onPress={closeMenu}>
          <Pressable onPress={() => {}} className="gap-2 px-3" style={{ paddingBottom: insets.bottom + 12 }}>
            <View className="overflow-hidden rounded-2xl bg-card">
              {moving ? (
                <>
                  <View className="border-b border-border/40 px-5 py-3">
                    <Text variant="muted" className="text-center">Move {menu?.recipe.name} to…</Text>
                  </View>
                  {moveTargets.map((s, i) => (
                    <Pressable key={s} onPress={() => void moveTo(s)} className={cn("px-5 py-4", i < moveTargets.length - 1 && "border-b border-border/40")}>
                ...
                <>
                  <View className="border-b border-border/40 px-5 py-3">
                    <Text variant="muted" className="text-center">{menu?.recipe.name}</Text>
                  </View>
                  <Pressable onPress={() => void changeMeal()} className="flex-row items-center justify-center gap-3 border-b border-border/40 px-5 py-4">
                    <SymbolView name={CHANGE_ICON} tintColor={iconColor} size={18} />
                    <Text>Change meal</Text>
                  </Pressable>
                  <Pressable onPress={() => setMoving(true)} className="flex-row items-center justify-center gap-3 border-b border-border/40 px-5 py-4">
                    <SymbolView name={MOVE_ICON} tintColor={iconColor} size={18} />
                    <Text>Move to…</Text>
                  </Pressable>
                  <Pressable onPress={() => void skipMeal()} className="flex-row items-center justify-center gap-3 px-5 py-4">
            ...
            <Pressable onPress={closeMenu} className="items-center rounded-2xl bg-card py-4">
      ...
      <Modal visible={jumpOpen} transparent animationType="fade" onRequestClose={() => setJumpOpen(false)}>
        <Pressable className="flex-1 items-center justify-center bg-black/40 px-8" onPress={() => setJumpOpen(false)}>
          <Pressable onPress={() => {}} className="w-full overflow-hidden rounded-2xl border border-border bg-card">
            <ScrollView className="max-h-96">
              {dates.map((d) => {
                const key = dateKey(d);
                const hasPlan = hasPlanOn(key);
                return (
                  <Pressable
                    key={key}
                    onPress={() => {
                  ...
                  >
                    <Text className={cn(key === selected && "font-semibold")}>
                      {WEEKDAY_LONG[d.getDay()]}, {d.getDate()} {MONTH_SHORT[d.getMonth()]}
                    </Text>
                    {key === todayKey ? <Text variant="muted">Today</Text> : hasPlan ? <View className="size-1.5 rounded-full bg-muted-foreground/60" /> : null}
                  </Pressable>
                );
              })}
            </ScrollView>
      ...
      >
        <Pressable className="flex-1 items-center justify-center bg-black/40 px-8" onPress={() => setImportSlot(null)}>
          <Pressable onPress={() => {}} className="w-full gap-3 rounded-2xl border border-border bg-card p-6">
            <Text variant="large">Import for {importSlot ? SLOT_LABEL[importSlot] : ""}</Text>
            <Text variant="muted">Paste a recipe URL — it will be imported and planned for {selected}.</Text>
            <Input
              value={importUrl}
            ...
            />
            {importError ? <Text variant="small" className="text-destructive">{importError}</Text> : null}
            <View className="flex-row gap-3">
              <Button variant="outline" className="flex-1" onPress={() => setImportSlot(null)}>
                <Text>Cancel</Text>
              </Button>
              <Button className="flex-1" onPress={() => void doImport()} disabled={importing || !importUrl.trim()}>
      ...
      >
        <Pressable className="flex-1 items-center justify-center bg-black/40 px-6" onPress={() => setCookbookSlot(null)}>
          <Pressable onPress={() => {}} className="w-full max-h-[70%] overflow-hidden rounded-2xl border border-border bg-card">
            <View className="border-b border-border/40 p-4">
              <Text variant="large">Cookbook → {cookbookSlot ? SLOT_LABEL[cookbookSlot] : ""}</Text>
              <Text variant="muted">{variants.length ? "Tap to plan" : "No variants yet — import one first."}</Text>
            </View>
            <ScrollView contentContainerClassName="gap-2 p-3">
              {variants.map((r) => (
                <Pressable
                  key={r.id}
                  onPress={() => {
                    if (cookbookSlot) void onPlan(cookbookSlot, { id: r.id, name: r.name ?? "…" });
              ...
              {variants.length === 0 ? (
                <Button variant="outline" onPress={() => { setCookbookSlot(null); setImportSlot(cookbookSlot); }}>
