...
Add native toolbar items to Stack screens. Items can be placed in the header (left/right) or in a bottom toolbar area.

**Important:** Available in Expo SDK 55+.
...
- Icon button (iOS SF Symbol): `<Stack.Toolbar.Button icon="star.fill" onPress={() => {}} />`
- Icon button (Android / Image source): `<Stack.Toolbar.Button icon={require("./assets/star.png")} onPress={() => {}} />` or `<Stack.Toolbar.Button onPress={() => {}}><Stack.Toolbar.Icon src={require("./assets/star.png")} /></Stack.Toolbar.Button>`
- Text button: `<Stack.Toolbar.Button onPress={() => {}}>Done</Stack.Toolbar.Button>`

> **Android Icons:** `Stack.Toolbar.Button` on Android requires an `ImageSourcePropType` icon. SF Symbols and xcasset icons are not supported. Use the `icon` prop with a `require()` or `{ uri }` source, or use `<Stack.Toolbar.Icon src={...} />`.
...
- On Android, `Stack.Toolbar.Button` does not support SF Symbols or xcassets — requires `ImageSourcePropType` (`require()`, `{ uri }`, or `<Stack.Toolbar.Icon src={...} />`)
